...
### 1. Clone Repository
```bash
git clone https://github.com/nulsec/Telegram-Bot-Get-OTP-From-Target.git
cd Telegram-Bot-Get-OTP-From-Target
```
...
voip_project_id: "your_project_id"
voip_api_token: "your_api_token"
voip_space_url: "your_space.provider.com"
voip_phone: "+1234567890"
...
# Telegram Bot
telegram_api_token: "bot_token_from_botfather"
telegram_api1_token: "second_bot_token_for_channel"
telegram_channelid: "-1001234567890"
...
### 4. Database Setup
Database is auto-created on first run. Tables include:
- `members` - User accounts and credits
- `vouchers` - Access codes
- `signalwire_phones` - Phone number inventory
- `logs`, `logs_cvv`, `logs_pin`, etc. - Call logs
- `saved_scripts` - Custom call scripts

### 5. Gunicorn Configuration

For **4 Core / 16GB RAM**:
```bash
# Configuration already optimized in gunicorn_config.py
```

For **8 Core / 32GB RAM**:
```bash
python update_config_8core.py
```
...
| Configuration | Workers | Threads | Audio Workers | API Workers | Throughput |
|--------------|---------|---------|---------------|-------------|------------|
| 4 Core / 16GB | 1 | 16 | 32 | 16 | ~130 req/sec |
| 8 Core / 32GB | 1 | 32 | 64 | 32 | ~250-300 req/sec |
